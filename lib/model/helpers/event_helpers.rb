  ...
  def self.add_geojson_events(geojson)
    ...
    upload["features"].each do |feature|
      ...
      feature["properties"].keys.each do |property|
        import_property(property, event)
      end
      Event.create(event)
    end
  end

  def import_property(property, event)
    if property =~ /date/
      import_date_property(property, event)
    elsif property == "tags"
      import_tag(property, event)
    elsif property == "events"
      import_linked_events(property, event)
    else
      event[property.to_sym] = feature["properties"][key]
    end
  end

  def import_date_property(property, event)
    event[property.to_sym] = DateTime.new(feature["properties"][property].to_i)
  end

  def import_tag(property, event)
    event[property.to_sym] = feature["properties"][property].map do |tag|
      Tag.first_or_create(name: tag)
    end
  end

  def import_linked_events(linked_events, event)
